...
A CRaft‑inspired tool that analyzes an LLM’s chain‑of‑thought (CoT) in real time to detect **manipulative reasoning patterns** (evaluation awareness, deception planning, safety evasion) and outputs a **0–100 risk score** with a Streamlit dashboard.[file:30][file:1]
...
- **CRaft‑style risk scoring**
  - Text‑only approximation of CRaft Stage A/E:
    - `pattern_risk` (0–1) from template overlap.
...
- **Two modes**
  ...
  - `audit_aware`: model is told it is under audit, to expose strategies and evaluation awareness (evasion‑style setting).[file:30][file:1]
- **Streamlit dashboard**
